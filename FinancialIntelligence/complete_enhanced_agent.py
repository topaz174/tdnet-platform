"""
Enhanced Complete LangGraph-based agent framework for financial analytics
Integrates with the Enhanced Retrieval System for superior performance
"""
...
class EnhancedFinancialEntityExtractor:
    """Enhanced extractor that combines original logic with enhanced knowledge base"""
    
    def __init__(self):
        self.kb = FinancialKnowledgeBase()
        
        # Keep original patterns for compatibility
    ...
    def extract_entities(self, query: str) -> FinancialQuery:
        """Enhanced entity extraction combining original logic with knowledge base"""
        query_lower = query.lower()
        
        # First, use the enhanced knowledge base for classification
        enhanced_classification = self.kb.classify_query(query)
        ...
        # Add companies detected by enhanced system
        companies.extend(enhanced_classification.companies)
        
        # Remove duplicates while preserving order
        companies = list(dict.fromkeys(companies))
        
        # Only look for stock symbols if they appear to be intentional
        ...
        # Combine original financial metrics with enhanced ones
        metrics = [metric for metric in self.financial_metrics if metric in query_lower]
        metrics.extend(enhanced_classification.financial_terms)
        metrics = list(dict.fromkeys(metrics))  # Remove duplicates
        
        # Use enhanced classification for query type, fallback to original
        query_type = self._map_enhanced_to_original_type(enhanced_classification.query_type)
        if query_type == 'general':
            query_type = self._classify_query_type(query_lower)
        
        # Detect comparison type
        comparison_type = None
        if enhanced_classification.query_type == "comparative":
            comparison_type = 'company_comparison'
        elif enhanced_classification.query_type == "temporal":
    ...
    def _map_enhanced_to_original_type(self, enhanced_type: str) -> str:
        """Map enhanced classification types to original types"""
        mapping = {
            "temporal": "general",
        ...
        return mapping.get(enhanced_type, "general")
    
    def _classify_query_type(self, query: str) -> str:
        """Original classification logic as fallback"""
...
class EnhancedDocumentSearchTool(BaseTool):
    """Enhanced tool for searching financial documents"""
    
    name: str = "enhanced_document_search"
    description: str = "Search financial disclosure documents using enhanced semantic similarity with smart filtering"
    ...
    async def _arun(self, query: str, filters: Dict[str, Any] = None) -> List[RetrievalResult]:
        """Async implementation of enhanced document search"""
    ...
    def _run(self, search_results: List[RetrievalResult], data_type: str) -> Dict[str, Any]:
        ...
        for result in search_results:
            ...
            doc_info = {
                ...
                "classification_l1": result.classification_l1,
                "classification_l2": result.classification_l2
            }
...
class CompleteEnhancedFinancialAgent:
    """Enhanced version of the complete financial agent with advanced retrieval"""
    
    def __init__(self, retrieval_system: EnhancedFinancialRetrievalSystem, llm: ChatOpenAI):
        self.retrieval_system = retrieval_system
        self.llm = llm
        self.entity_extractor = EnhancedFinancialEntityExtractor()
        self.logger = logging.getLogger(__name__)
        
        # Initialize tools
        self.tools = [
            EnhancedDocumentSearchTool(retrieval_system),
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        workflow.add_node("enhanced_search", self._enhanced_search)
        ...
        workflow.add_edge("parse_query", "enhanced_search")
        workflow.add_edge("enhanced_search", "extract_data")
    ...
    async def _parse_query(self, state: AgentState) -> AgentState:
        """Enhanced query parsing with knowledge base integration"""
        query = state["query"]
        
        # Use enhanced knowledge base for initial classification
        enhanced_classification = self.retrieval_system.kb.classify_query(query)
        
        # Extract entities using enhanced extractor
        financial_query = self.entity_extractor.extract_entities(query)
        
        # Add reasoning step
        reasoning_step = f"Enhanced Parse: Type={enhanced_classification.query_type} -> {financial_query.query_type}, " \
                        f"Companies={financial_query.companies}, " \
                        f"Enhanced Terms={enhanced_classification.financial_terms}, " \
                        f"Metrics={financial_query.financial_metrics}, " \
                        f"Comparison={financial_query.comparison_type}"
        
        state["query_classification"] = enhanced_classification
        state["reasoning_steps"] = [reasoning_step]
        state["metadata"] = {
            "financial_query": financial_query.__dict__,
            "enhanced_classification": enhanced_classification.__dict__,
            "processing_start": datetime.now().isoformat()
        }
        
        return state
    
    async def _enhanced_search(self, state: AgentState) -> AgentState:
        """Enhanced document search with smart filtering"""
        query = state["query"]
        financial_query = state["metadata"]["financial_query"]
        enhanced_classification = state["metadata"]["enhanced_classification"]
        ...
        state["reasoning_steps"].append(
            f"Enhanced Search: Found {len(search_results)} documents using "
            f"enhanced retrieval with query expansion and smart filtering"
        )
        
        return state
    
    async def _extract_data(self, state: AgentState) -> AgentState:
        """Extract structured data from enhanced search results"""
        ...
        state["reasoning_steps"].append(
            f"Enhanced Extraction: Extracted {data_type} from {len(extracted_data)} companies "
            f"with classification information"
        )
        
        return state
    
    async def _perform_calculations(self, state: AgentState) -> AgentState:
        """Perform financial calculations with enhanced data"""
        ...
        # Determine calculation type based on enhanced classification
        if enhanced_classification["query_type"] == "comparative" or financial_query["comparison_type"] == "company_comparison":
            calc_data = {"companies": {}}
        ...
        elif enhanced_classification["query_type"] == "temporal" or financial_query["comparison_type"] == "change_analysis":
        ...
        state["reasoning_steps"].append(
            f"Enhanced Calculations: Performed {len(calculations)} calculations "
            f"based on enhanced query classification"
        )
        
        return state
    
    async def _generate_response(self, state: AgentState) -> AgentState:
        """Generate enhanced response with improved context"""
        query = state["query"]
        ...
        context = {
            ...
            "query_classification": {
                "type": enhanced_classification["query_type"],
                "financial_terms": enhanced_classification["financial_terms"],
                "expanded_query": enhanced_classification["expanded_query"][:200] + "..."
            },
            ...
            "detailed_results": [
                {
                    ...
                    "score": r.score,
                    "context": r.ctx[:400] + "..." if len(r.ctx or "") > 400 else r.ctx or "",
                    "classification": r.classification_l1,
                    "subcategory": r.classification_l2
                }
        ...
        prompt = f"""
        Based on the comprehensive enhanced financial analysis performed, provide a detailed response to the user's query.
        
        User Query: {query}
        
        Enhanced Analysis Context: {json.dumps(context, indent=2, default=str)}
        
        IMPORTANT INSTRUCTIONS:
        1. Use ONLY the company information provided in the enhanced analysis context
        2. List ALL companies found in the search results with their exact names and codes
        3. Leverage the enhanced query classification and expanded search terms used
        4. Reference document classifications and similarity scores when relevant
        5. Do not invent or assume company names not explicitly provided
        
        Please provide:
        1. A direct answer listing ALL companies that match the query criteria
        2. Enhanced insights from the retrieved documents using classification information
        3. Relevant financial metrics and trends from the advanced analysis
        4. Supporting evidence from the specific documents found with their classifications
        5. Any limitations or caveats about the analysis
        
        Format the response professionally and ensure all {len(search_results)} companies found are properly represented.
        Highlight how the enhanced retrieval system improved the search quality.
        """
    ...
    async def process_query(self, query: str) -> Dict[str, Any]:
        """Process a financial query using enhanced retrieval and return comprehensive results"""
        try:
            # Initialize state
            ...
            return {
                ...
                "enhanced_classification": final_state["query_classification"].__dict__ if final_state["query_classification"] else None,
...
async def main():
    """Example usage of the enhanced complete financial agent"""
    ...
    config = EnhancedRetrievalConfig(
        ...
        redis_url=os.getenv("REDIS_URL", "")  # Empty string to disable Redis if not available
    )
    ...
    agent = CompleteEnhancedFinancialAgent(retrieval_system, llm)
    ...
    for query in test_queries:
        ...
        if result["success"]:
            ...
            print(f"Enhanced query type: {result['enhanced_classification']['query_type'] if result['enhanced_classification'] else 'N/A'}")
